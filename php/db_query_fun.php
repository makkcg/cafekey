...
switch ($ordr) {// swich the order cases and create query based on the order
	...
	case 1: // get all the stock names 
       $sql="SELECT * FROM  `st_names` WHERE 1";//`st_id`!=5
	   $result = mysqli_query($con,$sql);
	   $sql_results = array();
	   while ($row = $result->fetch_row())
  	{
...
$sql="CALL `getnextID` ('".$param[1]."','mokacafeop')";//// mokacafe is the database name will be changed when online
	...
	break;
	case 2:
	   $sql="SELECT `itm_id`,`itm_shortname`,`itm_limit` FROM `stock_items` WHERE `itm_type`!=4";
	break;
	case 3:
	...
	break;
	   ...
	   while ($row = $result->fetch_row())
  	{
		...
		$Fld2=$row[1];
		$Fld3=$row[2];	
		$sql_results[]=array("itm_id" => $Fld1, "itm_name" => $Fld2,"itm_limit" => $Fld3);
	}
    break; 
   ...
   break;
	...
	case 8: // get the stock item types list  
       $sql="SELECT * FROM `st_items_types` WHERE `st_itm_id`!=4 And `st_itm_id`!=3";
	   $result = mysqli_query($con,$sql);
	   $sql_results = array();
	   while ($row = $result->fetch_row())
  	{
	...
	break;
	case 15: // alternative SP to save new stock item , updates all related tables (profile, cash, stock mov, stock itmes, acc mov, purch)
	////`buy_st_item_new` 
		/////parameters : boughtby int = cashier 0 or profileid 1
	//itmsname,itmlname,itm_type , itm_desc text ,itm_price , userid , itm_qnty_unit , groupid , st_itm_mov_typ , st_itm_mov_cur_stid  , st_itm_mov_qnty , st_itm_mov_dat , st_itm_mov_time , st_itm_mov_notes , st_itm_mov_to_st , buyerid, stock item id for existing items only )
$sql="CALL `buy_st_item` ('".$param[1]."','".$param[2]."','".$param[3]."','".$param[4]."','".$param[5]."','".$param[6]."','".$param[7]."','".$param[8]."','".$param[9]."','".$param[10]."','".$param[11]."','".$param[12]."','".$param[13]."','".$param[14]."','".$param[15]."','".$param[16]."','".$param[17]."','".$param[18]."')";////
	   $result = mysqli_query($con,$sql);
...
$sql="CALL `get_stok_item_balance` ('".$param[1]."','".$param[2]."','".$param[3]."','".$param[4]."','".$param[5]."')";
	$result = mysqli_query($con,$sql);
	   $sql_results = array();
	   if($param[5]=='1'){
	...
	break;
	case 17: // get all profiles whom can buy stock ( admin, owners,staff) 
       $sql="SELECT `items_ingradients`.`item_ingradients_id`,`stock_items`.`itm_shortname`,`items_ingradients`.`itm_ingrad_qnty`,`st_item_qnty_unit`.`st_itm_qnty_unit_name` FROM `items_ingradients` Inner Join `stock_items` on `stock_items`.`itm_id`=`items_ingradients`.`itm_ingrad_id` Inner Join `st_item_qnty_unit` on `st_item_qnty_unit`.`st_itm_qnty_unit_id`=`items_ingradients`.`itm_ingrad_qnty_unit`  WHERE `items_ingradients`.`itm_id`='".$param[1]."';";
	   $result = mysqli_query($con,$sql);
	   $sql_results = array();
	   while ($row = $result->fetch_row())
  	{
