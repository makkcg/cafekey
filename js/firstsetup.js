...
///fucntion to remove item or all items 9999 from any array
function removearrayitems(arr,itemtoremorAll9999){
	switch(itemtoremorAll9999){
	case 9999:
	...
	var arritmindex=parseInt(itemtoremorAll9999)
	var arrlength=arr.length
	for(ii=0;ii<arrlength;ii++){
		itmtoremove=arr[ii].arritmindex
		itmtoremove=parseInt(itmtoremove);
		////alert("ii "+ii+" value "+value+"  ingitmid "+ingritmid)
	
	if(itmtoremove==arritmindex){
		...
		//drawstockmeasurmentsunits()
	}
	return false;
	//arr.splice(itemtoremorAll9999,1)
	break;	
	}
}
////////////First Time Setup for system///////////////////////////
///first time setup will be stored on the fly on an object and at the last step will execute the object into DB
function ftobj(){
this.dat=null
this.time=null
this.stockitems=[]
...
this.stockitmBalance=[]
this.profBalances=[]
this.expenceslist=[]
this.users=[]	
}

function initializefirstsetupobj(){

firsttimesetupobj= new ftobj;
firsttimesetupobj.dat=frmtdatetimesql(1)
firsttimesetupobj.time=frmtdatetimesql(2)
///fields for each array item

///stitmUnits={st_itm_qnty_unit_id:0,st_itm_qnty_unit_name:0,st_itm_qnty_unit_desc:0}

///stockitems={itm_id:0,itm_shortname:0,itm_longname:0,itm_type:0,itm_data:0,itm_sell_price:0,itm_reg_date:0,itm_userid:0,itm_timestamp:0,itm_qnty_unit:0,group_id:0,itm_limit:0} ///types 1 osol,2 row material, 3 row for sale items, 4 sales menu item

///stocks={st_id:0,st_name:0,st_desc:0}

///salesItmes={itm_id:0,itm_shortname:0,itm_longname:0,itm_type:0,itm_data:0,itm_sell_price:0,itm_reg_date:0,itm_userid:0,itm_timestamp:0,itm_qnty_unit:0,group_id:0,itm_limit:0,itemingr:[]} /----//itemingr[]={item_ingradients_id:0,itm_id:0,itm_ingrad_id:0,itm_ingrad_qnty:0,itm_ingrad_qnty_unit:0,itm_updt_date:0,itm_timestamp:0,itm_userid:0,itm_ingr_cost:0}


///profusers={user_id:0,profileid:0,fullname:0,mobile:0,mobile2:0,email:0,address:0,jobtitle:0,type:0,phone:0,notes:0}-//prof types:1 admin, 2 partner/owner , 3 staff,4 client,5 system prof

///users={id:0,username:0,ip:0,user_type:0,permission:0,Last_Login:0,Registration_Date:0,Expiration_Date:0,userlogin_flag:0,sessionDB_ID:0,password:0}

///cashbalance={cash_id:0,acc_mov_id:0,cash_dat:0,cash_time:0,cash_timestamp:0,acc_mov_type:0,cash_openbalance:0,cash_value:0,cash_endbalance:0,cash_desc:0}

///stockitmBalance={st_itm_mov_id:0,st_itm_mov_typ:0,st_itm_mov_usrid:0,st_itm_mov_itmid:0,st_itm_mov_cur_stid:0,st_itm_mov_qnty:0,st_itm_mov_qnty_unit:0,st_itm_mov_dat:0,st_itm_mov_timstmp:0,st_itm_mov_notes:0,st_itm_mov_to_st:0,st_itm_mov_st_itm_mov_price:0,st_itm_mov_buyerid:0,st_itm_mov_profid:0,st_itm_mov_itm_typ:0}

///profBalances={acc_prof_mov_id:0,acc_mov_id:0,prof_mov_dat:0,prof_mov_time:0,prof_mov_timestamp:0,prof_id:0,acc_mov_type:0,prof_mov_value:0,prof_mov_balance:0,prof_mov_desc:0,salary:0}

///expenceslist={exp_id:0,exp_name:0,top_level_id:0,localid:0}-///localid is used to form the tree internally in the object

}
///profusers.push({itmid:itmid,itm_lname:itm_lname,itm_desc:itm_desc,itm_price:itm_price,itm_qnty:1,itm_total:(parseFloat(itm_price)).toFixed(2)})
function initializesetupprocess(){
$(".sub-stock-page").hide()
$(".st_items_subpage").hide()

document.currentstep=0
initializefirstsetupobj();
///load last saved setup
//initializeSetps(1)
}

function initializeSetps(stepNo){
	///hide all sub pages
	$(".sub-stock-page").hide();
	switch(stepNo){
	case 1:
	////show the units sub page
		$(".st_units_subpage").show();
	///clear the object
	removearrayitems(firsttimesetupobj.stitmUnits,9999)
	///clear the GUI
	$(".st_itm_qnty_unit_rows").find(".st_itm_qnty_unit_row").remove();
	$(".iniResettxt").val("");
	document.currentstep=1
	///retrive current data from DB
	$.ajaxSetup({async:false});//get all items limits
$.post("php/firsttimesetup.php",{jsonobj: JSON.stringify(firsttimesetupobj),step:"0~1",param3:"1"},function(server_response) {  
				data = $.parseJSON(server_response);              
                 firsttimesetupobj.stitmUnits.splice(0,firsttimesetupobj.stitmUnits.length)
				// alert(server_response)
				console.log(server_response)
				 
				// data=server_response
				 for(ii=0;ii<data.length;ii++){
					 console.log(data[ii].st_itm_qnty_unit_id)
					 firsttimesetupobj.stitmUnits.push({"st_itm_qnty_unit_id":data[ii].st_itm_qnty_unit_id,"st_itm_qnty_unit_name":data[ii].st_itm_qnty_unit_name,"st_itm_qnty_unit_desc":data[ii].st_itm_qnty_unit_desc,"arritmindex":ii})
				 }
				 drawstockmeasurmentsunits()
	...
	////show the stock items sub page
	
	$(".st_items_subpage").show();
	 ///clear the object
	removearrayitems(firsttimesetupobj.stockitems,9999)
	///clear the GUI
	$(".st_itms_rows").find(".st_itms_row").remove();
	$(".iniResettxt").val("");
	
	////populate measuremnts units
	//unit_measure
	$("#unit_measure").find('option').remove()
	for(os=0;os<firsttimesetupobj.stitmUnits.length;os++){
		$("#unit_measure").append("<option value='"+firsttimesetupobj.stitmUnits[os].st_itm_qnty_unit_id+"' data-unitname='"+firsttimesetupobj.stitmUnits[os].st_itm_qnty_unit_name+"' data-unit desc='"+firsttimesetupobj.stitmUnits[os].st_itm_qnty_unit_desc+"' data-unitid='"+firsttimesetupobj.stitmUnits[os].st_itm_qnty_unit_id+"' >"+firsttimesetupobj.stitmUnits[os].st_itm_qnty_unit_name+"</option>")
	}/////
	
	////populate stock item types from db
	//// delete all options and regenerate them
	$("#itm_type_dd").find('option').remove();

$.ajaxSetup({async:false});
$.post("php/db_query_fun.php",{param:'8~'},function(server_response) {
					data = $.parseJSON(server_response);
					$("#itm_type_dd").append('<option value="0">اختر نوع الصنف</option>');
                  	
					for(var i = 0; i < data.length; i++){
                  	$("#itm_type_dd").append('<option data-itmtypeid='+data[i].itm_type_id+' value='+data[i].itm_type_id+'  itm_type_desc="'+data[i].itm_type_desc+'">'+data[i].itm_type_name+'</option>');
					
                  	}
});
	
	///retrive current data from DB
	//{arritmindex:0,itm_id:0,itm_shortname:0,itm_longname:0,itm_type:0,itm_data:0,itm_sell_price:0,itm_reg_date:0,itm_userid:0,itm_timestamp:0,itm_qnty_unit:0,group_id:0,itm_limit:0} ///types 1 osol,2 row material, 3 row for sale items, 4 sales menu item
	
	$.ajaxSetup({async:false});//get all stock items
$.post("php/firsttimesetup.php",{jsonobj: JSON.stringify(firsttimesetupobj),step:"0~1",param3:"2"},function(server_response) {  
				data = $.parseJSON(server_response);              
                 firsttimesetupobj.stockitems.splice(0,firsttimesetupobj.stockitems.length)
				// alert(server_response)
				console.log(server_response)
				...
				// data=server_response
				 for(ii=0;ii<data.length;ii++){
					 ...
					 itm_typename=$("#itm_type_dd").find("option[data-itmtypeid="+data[ii].itm_id+"]").text()
					 itm_qnty_unitname=$("#unit_measure").find("option[data-unitid="+data[ii].itm_qnty_unit+"]").text()
					 firsttimesetupobj.stockitems.push({"arritmindex":ii,"itm_id":data[ii].itm_id,"itm_shortname":data[ii].itm_shortname,"itm_longname":data[ii].itm_longname,"itm_type":data[ii].itm_type,"itm_typename":itm_typename,"itm_data":data[ii].itm_data,"itm_sell_price":data[ii].itm_sell_price,"itm_reg_date":data[ii].itm_reg_date,"itm_userid":data[ii].itm_userid,"itm_timestamp":data[ii].itm_timestamp,"itm_qnty_unit":data[ii].itm_qnty_unit,"itm_qnty_unitname":itm_qnty_unitname,"group_id":data[ii].group_id,"itm_limit":data[ii].itm_limit})
				 }
				 drawstockitems()
...
/// function to draw stock measurments units in the html from the object

function drawstockmeasurmentsunits()
{
	///recalculate stitmUnits.arritmindex for all
for(pp=0;pp<firsttimesetupobj.stitmUnits.length ;pp++){
	firsttimesetupobj.stitmUnits[pp].arritmindex=pp
}
	$(".st_itm_qnty_unit_rows").find(".st_itm_qnty_unit_row").remove()
///show the added item in the screen div
for(aa=0;aa<firsttimesetupobj.stitmUnits.length;aa++){
$(".st_itm_qnty_unit_rows").append("<div class='frmfieldrow100 st_itm_qnty_unit_row' data-arridrow='"+(aa)+"'><div class='inpboxtitle ' style='font-wieght:bold;'>الوحدة : </div>	<div class='inpboxtitle st_itm_qnty_unit_name'>"+firsttimesetupobj.stitmUnits[aa].st_itm_qnty_unit_name+"</div><div class='inpboxtitle ' style='font-wieght:bold;'>الوصف  : </div><div class='inpboxtitle st_itm_qnty_unit_desc'>"+firsttimesetupobj.stitmUnits[aa].st_itm_qnty_unit_desc+"</div><div class='removeStitmUnitbtn inpboxtitle'><a data-arrid='"+(aa)+"' data-arritmindex='"+(firsttimesetupobj.stitmUnits[aa].arritmindex)+"'><img align='middle' class='ingr_remove' src='images/del24.png' ></a></div></div>")	
}//end for
}

function drawstockitems()
{
	///recalculate stitmUnits.arritmindex for all
for(pp=0;pp<firsttimesetupobj.stockitems.length ;pp++){
	firsttimesetupobj.stockitems[pp].arritmindex=pp
}
	$(".st_itms_rows").find(".st_itms_row").remove()
///show the added item in the screen div
/////{arritmindex:0,itm_id:0,itm_shortname:0,itm_longname:0,itm_type:0,itm_typename:0,itm_data:0,itm_sell_price:0,itm_reg_date:0,itm_userid:0,itm_timestamp:0,itm_qnty_unit:0,itm_qnty_unitname:0,group_id:0,itm_limit:0} ///types 1 osol,2 row material, 3 row for sale items, 4 sales menu item

for(aa=0;aa<firsttimesetupobj.stockitems.length;aa++){
$(".st_itms_rows").append("<div class='frmfieldrow100 st_itms_row' data-arridrow='"+(aa)+"'><div class='inpboxtitle ' style='font-wieght:bold;'>كود:</div>	<div class='inpboxtitle itm_id'>"+firsttimesetupobj.stockitems[aa].itm_id+"</div><div class='inpboxtitle ' style='font-wieght:bold;'>اسم الصنف:</div><div class='inpboxtitle itm_shortname'>"+firsttimesetupobj.stockitems[aa].itm_shortname+"</div><div class='inpboxtitle ' style='font-wieght:bold;'>الاسم الكامل:</div><div class='inpboxtitle itm_longname'>"+firsttimesetupobj.stockitems[aa].itm_longname+"</div><div class='inpboxtitle ' style='font-wieght:bold;'>نوع الصنف:</div><div class='inpboxtitle itm_typename' data-itm_type='"+firsttimesetupobj.stockitems[aa].itm_type+"'>"+firsttimesetupobj.stockitems[aa].itm_typename+"</div>    <div class='inpboxtitle ' style='font-wieght:bold;'>الحد الأدنى:</div><div class='inpboxtitle itm_limit'>"+firsttimesetupobj.stockitems[aa].itm_limit+"</div><div class='inpboxtitle ' style='font-wieght:bold;'>الوحدة:</div><div class='inpboxtitle itm_qnty_unitname'>"+firsttimesetupobj.stockitems[aa].itm_qnty_unitname+"</div><div class='inpboxtitle ' style='font-wieght:bold;'>تفاصيل:</div><div class='inpboxtitle itm_data'>"+firsttimesetupobj.stockitems[aa].itm_data+"</div><div class='removeStItmbtn' class='inpboxtitle' data-arrid='"+(aa)+"' data-arrname=''><a data-arrid='"+(aa)+"' data-arritmindex='"+(firsttimesetupobj.stockitems[aa].arritmindex)+"'><img data-arrid='"+(aa)+"' align='middle' class='stitm_remove' src='images/del24.png' ></a></div></div>")	
}//end for
}
$(document).ready(function() {
initializesetupprocess()
/////////////////////////////setup menu screen ////////////////////
$(".setupmenu li a").click(function(){//// user clicks on any of setup sub menu buttons
var menuitem=$(this).data("id")
	switch(parseInt(menuitem)){
	case 0: ///start first setup process
	initializesetupprocess();
	initializeSetps(1)
	...
	break;
	case 3://مسح قواعد البانات
$.post("php/import_clean_db.php",{param:'1~0'},function(server_response) {
						//data = $.parseJSON(server_response);
						alert(server_response)
})
	break;
...
////add stock items to the object
$("#st_itm_add_btn").click(function(){//// user clicks on any of setup sub menu buttons
...
var arritmindex=0
var itm_id=0
var itm_sell_price=0
var itm_reg_date=frmtdatetimesql(1)
var itm_userid=1
var itm_timestamp=frmtdatetimesql(0)
var group_id=0
var itm_shortname=$("#itm_shortname").val()
var itm_longname=$("#itm_longname").val()
var itm_data=$("#itm_data").val()
var itm_limit=$("#itm_limit").val()
//dropdown
var itm_type=$("#itm_type_dd").find(":selected").val();
var itm_typename=$("#itm_type_dd").find(":selected").text();
var itm_qnty_unit=$("#unit_measure").find(":selected").val();
var itm_qnty_unitname=$("#unit_measure").find(":selected").text();

firsttimesetupobj.stockitems.push({"itm_id":null,"itm_shortname":itm_shortname,"itm_longname":itm_longname,"arritmindex":(parseInt(firsttimesetupobj.stockitems.length)),"itm_sell_price":itm_sell_price,"itm_reg_date":itm_reg_date,"itm_userid":itm_userid,"itm_timestamp":itm_timestamp,"group_id":group_id,"itm_data":itm_data,"itm_limit":itm_limit,"itm_type":itm_type,"itm_typename":itm_typename,"itm_qnty_unit":itm_qnty_unit,"itm_qnty_unitname":itm_qnty_unitname})

//draw the inserted measurements unit to the html from the object
drawstockitems()
...
/////
////add stock measuremtn units to the object
$("#st_itm_qnty_unit_add_btn").click(function(){//// user clicks on any of setup sub menu buttons
var st_itm_qnty_unit_name=$("#st_itm_qnty_unit_name").val()
var st_itm_qnty_unit_desc=$("#st_itm_qnty_unit_desc").val()



firsttimesetupobj.stitmUnits.push({"st_itm_qnty_unit_id":null,"st_itm_qnty_unit_name":st_itm_qnty_unit_name,"st_itm_qnty_unit_desc":st_itm_qnty_unit_desc,"arritmindex":(parseInt(firsttimesetupobj.stitmUnits.length))})

//draw the inserted measurements unit to the html from the object
drawstockmeasurmentsunits()

///clear the inputs
$("#st_itm_qnty_unit_name").val("")
$("#st_itm_qnty_unit_desc").val("")
});
/////
///remove stock measurment unit item from the list
$(document).on('click','.removeStitmUnitbtn a',function(){
var selecteddiv=$(this).data("arrid")
var arritmindex=$(this).data("arritmindex")

removearrayitems(firsttimesetupobj.stitmUnits,arritmindex)
drawstockmeasurmentsunits()
});////

///remove stock item from the list
$(document).on('click','.removeStItmbtn a',function(){
var selecteddiv=$(this).data("arrid")
var arritmindex=$(this).data("arritmindex")

removearrayitems(firsttimesetupobj.stockitems,arritmindex)
drawstockitems()
});////


///save stock measurement list to DB and go to step2
$(document).on('click','#saveGotoStep2',function(){
	if(firsttimesetupobj.stitmUnits.length>0){
////save the list in the array firsttimesetupobj.stitmUnits to db
$.ajaxSetup({async:false});//get all items limits
$.post("php/firsttimesetup.php",{jsonobj: JSON.stringify(firsttimesetupobj),step:"1~0"},function(server_response) {  
				//data = $.parseJSON(server_response);              
                 firsttimesetupobj.stitmUnits.splice(0,firsttimesetupobj.stitmUnits.length)
				 //alert(data)console.log(data[ii].st_itm_qnty_unit_id)
				 console.log(server_response)
		...
		initializeSetps(2)
				...
				alert("لايوجد بيانات لحفظها، يرجى ادخال الوحدات")	 
				 };///end if
///hide step1 screen, update document.currentstep=2 , show step 2 window

});////

///save stock measurement list to DB and go to step2
$(document).on('click','#saveGotoStep3',function(){
	if(firsttimesetupobj.stockitems.length>0){
////save the list in the array firsttimesetupobj.stitmUnits to db
$.ajaxSetup({async:false});//get all items limits
$.post("php/firsttimesetup.php",{jsonobj: JSON.stringify(firsttimesetupobj),step:"2~0"},function(server_response) {  
				//data = $.parseJSON(server_response);              
                 firsttimesetupobj.stockitems.splice(0,firsttimesetupobj.stockitems.length)
				 //alert(data)console.log(data[ii].st_itm_qnty_unit_id)
				 console.log(server_response)
				 ...
				 data=server_response
				 for(ii=0;ii<data.length;ii++){
					 console.log(data[ii].itm_id)
					 firsttimesetupobj.stockitems.push({"itm_id":data[ii].itm_id,"itm_shortname":data[ii].itm_shortname,"itm_longname":data[ii].itm_longname,"arritmindex":(ii),"itm_sell_price":data[ii].itm_sell_price,"itm_reg_date":data[ii].itm_reg_date,"itm_userid":data[ii].itm_userid,"itm_timestamp":data[ii].itm_timestamp,"group_id":data[ii].group_id,"itm_data":data[ii].itm_data,"itm_limit":data[ii].itm_limit,"itm_type":data[ii].itm_type,"itm_typename":data[ii].itm_typename,"itm_qnty_unit":data[ii].itm_qnty_unit,"itm_qnty_unitname":data[ii].itm_qnty_unitname})
		...
		initializeSetps(3)
				...
				alert("لايوجد بيانات لحفظها، يرجى ادخال الاصناف")	 
				 };///end if
///hide step1 screen, update document.currentstep=2 , show step 2 window
...
$("#itm_type_dd").change(function() {
	var selecteditem_id = $('#itm_type_dd').find(":selected").val();
	var selecteditem_desc=$('#itm_type_dd').find(":selected").attr('itm_type_desc');
...
/////////////////////////////////by Mohammed Khalifa 01114442161//////
