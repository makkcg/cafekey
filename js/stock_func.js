...
break;
case 4:
$("#stock_salesItem").find('option').remove();
$.post("php/db_query_fun.php",{param:'4~5'},function(server_response) {
						data = $.parseJSON(server_response);
						$("#stock_salesItem").append(' <option value="0">اختر الصنف</option>');
						for(var i = 0; i < data.length; i++){
                  		$("#stock_salesItem").append('<option value='+data[i].itm_id+'>'+data[i].itm_name+'</option>');
						}//end for
});///end post
...
$("#stock_salesItem").change(function() {
	...
	if(operation_id==0){
		...
		$.post("php/db_query_fun.php",{param:'17~'+selecteditem_id},function(server_response) {
						...
						for(var i = 0; i < data.length; i++){
                  		$('.ing_rows').append('<div class="ing_row" data-dbrowid="'+data[i].ing_AI+'" style="width:100%;float:right;"><div class="inpboxtitle  st_ing_arrindex">'+(i+1)+'</div><div class="inpboxtitle  st_ing_name">'+data[i].ing_name+'</div><div class="inpboxtitle  st_ing_qnty" data-value="0">'+data[i].ing_qnty+'</div><div class="inpboxtitle  st_ing_unit" >'+data[i].ing_qnty_unit+'</div></div>');
						}//end for
});///end post
